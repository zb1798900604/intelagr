				...
				<td>
					...
					<table width="100%" border="0">
						...
						<tr>
							...
							<td align="left" width="25%">
								<fieldset class="fieldset_common_style">
									<input type="checkbox" name="processMode"  value="01"/>警告<input type="checkbox" name="processMode" value="02"/>责令停产停业
									<input type="checkbox" name="processMode" value="03"/>罚款   <br />
									<input type="checkbox" name="processMode"  value="04"/>暂扣或者吊销许可证、暂扣或者吊销执照
						...
						<tr>
							...
							&nbsp;&nbsp;产品名称：
							 <select id="productCode" name="productCode" style="width:170px;" class="easyui-combobox" data-options="editable:true">
						...
						<tr>
							...
							<td>
							拉黑时长：<s:select id='blackListTimeLimit' name="blackListTimeLimit" value="${blackListTimeLimit}" codeKey="BlackListDuration" entityName="commonData" hasPleaseSelectOption="false" width="170"/>
							</td>
						</tr>
						<tr>
...
<script type="text/javascript">
	...
	// 	Public.ajaxPost('/companyCPLT/getProductInfo?companyCode=' + companyCode,[], function(e) {
	// 		if (200 == e.status) {
	// 			initPro( e.data );
	// 		} else {
	...
	function save(){
		...
		a.reCallRecord={};
		a.reCallRecord.batchNo=a.batchNo;
		a.reCallRecord.productCode=a.productCode;
		a.reCallRecord.recallReason=a.recallReason;
		...
		Public.ajaxPost('/companyCPLT/saveSettle', JSON.stringify(a), function(e) {
			hideLoading();
			// alert(e.status);
			if (200 == e.status) {
	...
	function showUploadDialog(bizType, bizCode, title){
		$('#uploadDialog').dialog({
		    ...
		    closed: false,
		    cache: false,
		    href: '${pageContext.request.contextPath}/file/uploadInit?bizType='+bizType,
		    modal: true,
		    onClose : function(){
	...
	function showFileList( bizType, bizCode, title ){
		$.ajax({
			url: "${pageContext.request.contextPath}/file/fileList?bizType="+bizType+"&bizCode=",
			type: 'post',
			dataType: 'text',
	...
	function delFile(bizType, bizCode, filePath){
		...
		//bizCode 为空时，传空串
		Public.ajaxGet('${pageContext.request.contextPath}/file/deleteFile', {bizType : bizType, bizCode : bizCode, filePath : filePath},
			function(e) {
			if (200 == e.status) {
				//重新更新本地列表
